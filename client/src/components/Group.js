...
class Group extends React.Component {
  ...
  render () {
    ...
    .map((tut, index) =>
      <article key={index} className='center w5 br3 hidden pa3'>
        <div className={`br3 ba b--black-10 br--top ${tut.stuck ? 'bg-red' : 'bg-white'}`}>
          <a href={tut.link} target='_blank' className='dib link'>
            <h1 className='f4 br3 br--top black-60 mv0 pv2 ph3 truncate'>{tut.nickname}</h1>
          </a>
        </div>
        <div className='pa3 ba b--black-10 br--bottom br3'>
          <h2 className='f5 fw4 gray mt0 truncate'> {tut.title}</h2>
        </div>
      </article>);
